...
export default function RequestDetail() {
  ...
  if (!request) {
    return (
      <div className="p-6 max-w-4xl mx-auto">
        ...
        <button onClick={() => navigate('/requests')} className="mt-2 text-sm text-primary hover:underline font-semibold">Back to Register</button>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <button onClick={() => navigate('/requests')} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-primary mb-4 transition-colors font-semibold">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h1 className="text-2xl font-bold tracking-tight font-mono text-primary">{request.id}</h1>
            ...
            {STATUS_OPTIONS.filter((s) => s !== request.status).map((s) => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-bold rounded-md border transition-colors ${
                  s === 'Finalized' ? 'bg-primary text-primary-foreground hover:opacity-90' : 'hover:bg-primary/5 hover:border-primary/40'
                }`}
              >
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-5">
          <section className="bg-card rounded-lg border shadow-ink p-5">
            <h2 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Raw Request</h2>
          ...
          <section className="bg-card rounded-lg border shadow-ink p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                {request.aiNotes && !isFinalized && !isEditing && (
                  <button onClick={handleStartEdit} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary transition-colors font-semibold">
                    <Edit3 className="h-3 w-3" /> Edit
                  </button>
                )}
                {!isFinalized && (
                  <button onClick={handleGenerateAI} disabled={isGenerating}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-primary text-primary-foreground rounded-md hover:opacity-90 transition-opacity disabled:opacity-50 active:scale-[0.98]"
                  >
            ...
            {isGenerating && (
              <div className="animate-shimmer rounded-md p-4 border border-dashed border-primary/40 animate-pulse-border">
            ...
            {request.aiNotes && !isGenerating && !isEditing && (
              <div className="space-y-4">
                <div><h3 className="text-xs font-bold text-muted-foreground mb-1">Summary</h3><p className="text-sm">{request.aiNotes.summary}</p></div>
                <div><h3 className="text-xs font-bold text-muted-foreground mb-1">Details & Context</h3><p className="text-sm leading-relaxed">{request.aiNotes.details}</p></div>
                <div><h3 className="text-xs font-bold text-muted-foreground mb-1">Proposed Next Action</h3><p className="text-sm leading-relaxed">{request.aiNotes.proposedAction}</p></div>
                {request.aiNotes.suggestedTags.length > 0 && (
                  <div>
                    <h3 className="text-xs font-bold text-muted-foreground mb-1.5">Suggested Tags</h3>
            ...
            {isEditing && (
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={handleSaveEdit} className="px-3 py-1.5 text-xs font-bold bg-primary text-primary-foreground rounded-md hover:opacity-90">Save</button>
                  <button onClick={() => setIsEditing(false)} className="px-3 py-1.5 text-xs font-bold border rounded-md hover:bg-surface-raised">Cancel</button>
                </div>
              </div>
            )}
                  ...
                  <div key={f.id} className="flex items-start gap-3 py-2 border-b last:border-b-0">
                    <button onClick={() => toggleFollowUp(request.id, f.id)} disabled={isFinalized}
                      className={`mt-0.5 h-4 w-4 rounded border flex items-center justify-center transition-colors ${
                        f.completed ? 'bg-success border-success' : 'border-muted-foreground/30 hover:border-primary'
                      }`}
                    >
            ...
            {!isFinalized && (
              <div className="flex gap-2">
                <input type="text" value={newFollowUp} onChange={(e) => setNewFollowUp(e.target.value)}
                  ...
                  className="flex-1 px-3 py-2.5 text-sm bg-background border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
                />
                <button onClick={handleAddFollowUp} className="px-3 py-2.5 text-xs font-bold border rounded-md hover:bg-primary/5 transition-colors">
        ...
        {/* Sidebar */}
        <div className="space-y-4">
          <div className="bg-card rounded-lg border shadow-ink p-5 space-y-3">
          ...
          {request.tags.length > 0 && (
            <div className="bg-card rounded-lg border shadow-ink p-5">
...
function EditField({ label, value, onChange, textarea }: { label: string; value: string; onChange: (v: string) => void; textarea?: boolean }) {
  const cls = "w-full px-3 py-2.5 text-sm bg-background border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary";
  return (
    <div>
